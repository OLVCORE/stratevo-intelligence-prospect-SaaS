-- ==========================================
-- CRIAR FUNÇÃO process_qualification_job_sniper
-- ==========================================
-- Esta função é chamada pelo frontend mas não existe no banco
-- Baseada em process_qualification_job, mas sem referência a coluna icp_data que não existe
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Se não há setor_foco definido, não deve dar pontos "neutros"
      ...
      -- Se não há UF, não deve dar pontos "neutros"
      ...
      -- 🔥 FIX: Usar source_name do candidato (se disponível), senão usar do job
      INSERT INTO public.qualified_prospects (
        ...
        v_location_score,
        v_match_breakdown,
        COALESCE(v_candidate.source_name, v_job.source_file_name, v_job.job_name),
        now(),
        now()
      )
