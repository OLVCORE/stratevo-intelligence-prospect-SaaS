/**
 * 🎯 MATCH & FIT ENGINE - STRATEVO One (Deno-compatible)
 * 
 * MC4-EDGE: Versão Deno-friendly do Match & Fit Engine para uso em edge functions.
 * 
 * Baseado em: src/services/matchFitEngine.ts
 * 
 * Funcionalidades:
 ...
 * - Deno-compatible (sem dependências Node)
 */

// ==================== TIPOS / INTERFACES ====================

/**
 * Lead B2B consolidado (simplificado para Deno)
 */
export interface LeadB2BDeno {
...
/**
 * ICP Criteria (simplificado para Deno)
 */
export interface ICPCriteriaDeno {
  setores_alvo?: string[];
  cnaes_alvo?: string[];
  porte?: string[];
  regioes_alvo?: string[];
...
/**
 * ICP Persona (simplificado para Deno)
 */
export interface ICPPersonaDeno {
  decisor?: string | null;
...
/**
 * ICP Profile (simplificado para Deno)
 */
export interface ICPProfileDeno {
  id?: string;
  nome?: string;
...
/**
 * ICP completo (simplificado para Deno)
 */
export interface ICPDeno {
  profile?: ICPProfileDeno | null;
  persona?: ICPPersonaDeno | null;
  criteria?: ICPCriteriaDeno | null;
}

/**
 * Produto/Solução do portfólio do tenant
 */
export interface TenantProductDeno {
  id: string;
  nome: string;
  descricao?: string;
  categoria?: string;
 ...
 */
export interface MatchFitInputDeno {
  // Lead consolidado (B2B)
  lead: LeadB2BDeno | null;
  
  // ICP(s) do tenant
  icp: ICPDeno | null;
  
  // Portfólio do tenant (produtos/soluções)
  portfolio: TenantProductDeno[];
 ...
 */
export interface MatchScoreDeno {
  // Referência
 ...
 */
export interface MatchRecommendationDeno {
  // Identificação
  title: string;
  ...
  // Score relacionado
  relatedScore?: MatchScoreDeno;
}

/**
 * Resultado completo do Match & Fit Engine
 */
export interface MatchFitResultDeno {
  // Scores calculados
  scores: MatchScoreDeno[];
  
  // Recomendações geradas
  recommendations: MatchRecommendationDeno[];
 ...
 */
export function computeMatchScoresDeno(input: MatchFitInputDeno): MatchScoreDeno[] {
  console.log('[MatchFitDeno] Iniciando cálculo de scores', {
    hasLead: !!input.lead,
    hasIcp: !!input.icp,
    portfolioSize: input.portfolio.length,
  });

  const scores: MatchScoreDeno[] = [];

  // Validar dados mínimos
  if (!input.lead || !input.icp) {
    console.warn('[MatchFitDeno] Dados insuficientes para cálculo de scores');
    return scores;
  }
  ...
  if (icp.criteria) {
    const icpScore = calculateICPFitScoreDeno(lead, icp.criteria, icp);
    if (icpScore.score > 0) {
      scores.push({
        referenceType: 'icp',
  ...
  for (const product of input.portfolio) {
    if (product.ativo === false) continue; // Ignorar produtos inativos

    const productScore = calculateProductFitScoreDeno(lead, product, icp);
  ...
  console.log('[MatchFitDeno] Scores calculados', {
    total: scores.length,
 ...
 */
function calculateICPFitScoreDeno(
  lead: LeadB2BDeno,
  criteria: ICPCriteriaDeno,
  icp: ICPDeno
): { score: number; factors: string[]; breakdown?: MatchScoreDeno['breakdown'] } {
  if (!criteria) {
    return { score: 0, factors: [] };
  }

  const factors: string[] = [];
  const breakdown: MatchScoreDeno['breakdown'] = {
    sectorMatch: 0,
    cnaeMatch: 0,
    sizeMatch: 0,
    regionMatch: 0,
  ...
  if (lead.companySector && criteria.setores_alvo && criteria.setores_alvo.length > 0) {
  ...
  if (lead.cnae && criteria.cnaes_alvo && criteria.cnaes_alvo.length > 0) {
  ...
  if (lead.companySize && criteria.porte && criteria.porte.length > 0) {
  ...
  if (lead.companyRegion && criteria.regioes_alvo && criteria.regioes_alvo.length > 0) {
  ...
  if (lead.interestArea && icp.persona?.desejos && icp.persona.desejos.length > 0) {
 ...
 */
function calculateProductFitScoreDeno(
  lead: LeadB2BDeno,
  product: TenantProductDeno,
  icp: ICPDeno
): { score: number; factors: string[]; breakdown?: MatchScoreDeno['breakdown'] } {
  const factors: string[] = [];
  const breakdown: MatchScoreDeno['breakdown'] = {
 ...
 */
export function buildRecommendationsDeno(
  input: MatchFitInputDeno,
  scores: MatchScoreDeno[]
): MatchRecommendationDeno[] {
  console.log('[MatchFitDeno] Gerando recomendações', {
    scoresCount: scores.length,
  });

  const recommendations: MatchRecommendationDeno[] = [];
  ...
  for (const score of topScores) {
    ...
    if (score.referenceType === 'product') {
      ...
      const recommendation = buildProductRecommendationDeno(product, score, input);
      if (recommendation) {
        recommendations.push(recommendation);
      }
    } else if (score.referenceType === 'icp') {
      const recommendation = buildICPRecommendationDeno(score, input);
      if (recommendation) {
        recommendations.push(recommendation);
      }
    }
  ...
  console.log('[MatchFitDeno] Recomendações geradas', {
 ...
 */
function buildProductRecommendationDeno(
  product: TenantProductDeno,
  score: MatchScoreDeno,
  input: MatchFitInputDeno
): MatchRecommendationDeno | null {
  const lead = input.lead;
  if (!lead) return null;
 ...
 */
function buildICPRecommendationDeno(
  score: MatchScoreDeno,
  input: MatchFitInputDeno
): MatchRecommendationDeno | null {
  const icp = input.icp;
 ...
 */
export function summarizeMatchFitDeno(result: {
  scores: MatchScoreDeno[];
  recommendations: MatchRecommendationDeno[];
}): string {
 ...
 */
export function runMatchFitEngineDeno(input: MatchFitInputDeno): MatchFitResultDeno {
  console.log('[MatchFitDeno] Iniciando engine completo', {
  ...
  // Calcular scores
  const scores = computeMatchScoresDeno(input);

  // Gerar recomendações
  const recommendations = buildRecommendationsDeno(input, scores);

  // Gerar resumo executivo
  const executiveSummary = summarizeMatchFitDeno({ scores, recommendations });
  ...
  const metadata = {
    ...
    totalProductsEvaluated: input.portfolio.filter(p => p.ativo !== false).length,
  ...
  console.log('[MatchFitDeno] Engine concluído', {
    scoresCount: scores.length,
