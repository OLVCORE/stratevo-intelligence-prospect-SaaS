...
interface GenericProgressPhase {
  id: string;
  name: string;
...
interface GenericProgressBarProps {
  phases: GenericProgressPhase[];
  currentPhase?: string;
  elapsedTime?: number; // segundos totais
  title?: string;
}

export function GenericProgressBar({ 
  phases, 
  currentPhase,
  elapsedTime = 0,
  title = 'Progresso da Análise'
}: GenericProgressBarProps) {
  ...
  // Calcular largura de cada segmento (baseado no tempo estimado ou igual)
  const totalEstimatedTime = phases.reduce((sum, p) => sum + (p.estimatedTime || 1), 0);
  
  const getPhaseColor = (phase: GenericProgressPhase) => {
  ...
  const getPhaseWidth = (phase: GenericProgressPhase) => {
    if (totalEstimatedTime === 0) return 100 / totalPhases; // Distribuição igual
    const estimated = phase.estimatedTime || 1;
    return (estimated / totalEstimatedTime) * 100;
  };
