...
-- ==========================================
-- Versão 2: Corrige hook ICP + função realmente qualifica candidatos
-- Copie TODO este conteúdo e cole no SQL Editor do Supabase

-- PARTE 1: Corrigir função process_qualification_job para realmente processar
...
BEGIN
  ...
  -- Se já está concluído, retornar contadores existentes
  IF v_job.status = 'completed' THEN
    RETURN QUERY SELECT 
      COALESCE(v_job.processed_count, 0),
      COALESCE(v_job.enriched_count, 0),
      true,
      format('Job já processado anteriormente. Processados: %s, Qualificados: %s', 
        COALESCE(v_job.processed_count, 0), 
        COALESCE(v_job.enriched_count, 0));
    RETURN;
  END IF;
  ...
  LOOP
    BEGIN
      ...
      IF v_icp_profile IS NOT NULL AND v_icp_profile.setor_foco IS NOT NULL THEN
        ...
        END IF;
      ELSE
        v_fit_score := v_fit_score + 15; -- Sem critério específico, score neutro
      END IF;
      
      -- 2. Localização match (25%) - se ICP tiver preferência de região
      -- Por enquanto, dar score base se tiver UF
